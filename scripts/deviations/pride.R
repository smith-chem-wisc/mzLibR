...
R_TABLE[c("pride files", "pride ftp-files")] <- "(the returned data.frame itself)"
...
PARENT_OMISSIONS[c(
  ...
  "PrideProjectSearchResult.matched_fields", "PrideProjectSearchResult.as_dict", "pride.search"
)] <- c(
  ...
  "a column, `extension`, not a function",
  "meaningless in R: the data.frame is already the record",
  "parity debt: `pride search` arrives with the mzLib 1.0.592 port",
  "parity debt: `pride search` arrives with the mzLib 1.0.592 port",
  "parity debt: `pride search` arrives with the mzLib 1.0.592 port"
)
