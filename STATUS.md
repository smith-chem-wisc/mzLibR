...
- **`max_threads` defaults to 1**, not the wire's, pyMzLib's and mzLibRust's −1. It was chosen
  when multithreaded runs dropped MBR intensities nondeterministically (mzLib#1111); mzLib#1155
  fixed that inside the pinned bridge, so the runtime warning is gone. The default stays until −1
  is re-measured on mzLib's K562 pair.
